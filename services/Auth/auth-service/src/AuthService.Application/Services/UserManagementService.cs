...
public class UserManagementService(IUserRepository users, IRoleRepository roles, ICloudinaryService cloudinary) : IUserManagementService
{
    public async Task<UserResponseDto> UpdateUserRoleAsync(string userId, string roleName)
    {
        ...
        var isUserAdmin = user.UserRoles.Any(r => r.Role.Name == RoleConstants.ADMINISTRADOR_ROLE);
        if (isUserAdmin && roleName != RoleConstants.ADMINISTRADOR_ROLE)
        {
            var adminCount = await roles.CountUsersInRoleAsync(RoleConstants.ADMINISTRADOR_ROLE);
