    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        var defaultRole = await roleRepository.GetByNameAsync(RoleConstants.ALUMNO_ROLE);
        if (defaultRole == null)
        {
            throw new InvalidOperationException($"Rol por defecto '{RoleConstants.ALUMNO_ROLE}' no encontrado. Asegúrese de que la siembra se ejecute antes del registro.");
        }
    ...
    {
        var userRole = user.UserRoles.FirstOrDefault()?.Role?.Name ?? RoleConstants.ALUMNO_ROLE;
    ...
    private UserDetailsDto MapToUserDetailsDto(User user)
    {
        return new UserDetailsDto
        {
            ...
            Username = user.Username,
            ProfilePicture = _cloudinaryService.GetFullImageUrl(user.UserProfile?.ProfilePicture ?? string.Empty),
            Role = user.UserRoles.FirstOrDefault()?.Role?.Name ?? RoleConstants.ALUMNO_ROLE
        };
    }
